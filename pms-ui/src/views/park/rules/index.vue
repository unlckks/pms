  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="首段收费时间" prop="firstMin">
        <el-input
          v-model="queryParams.firstMin"
          placeholder="请输入首段收费时间"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="rulesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="首段收费金额(元)" align="center" prop="firstMoney" />
      <el-table-column label="超过首段间隔时间" align="center" prop="afterStepMin" />
      <el-table-column label="超过首段收费金额(元)" align="center" prop="afterStepMoney" />
      <el-table-column label="尾段开始时间" align="center" prop="lastMin" />
      <el-table-column label="尾段间隔时间" align="center" prop="lastStepMin" />
      <el-table-column label="尾段间隔收费(元)" align="center" prop="lastStepMoney" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="首段收费金额" prop="firstMoney">
          <el-input v-model="form.firstMoney" placeholder="请输入首段收费金额" />
        ...
        <el-form-item label="尾段间隔收" prop="lastStepMoney">
...
<script>
import { listRules, getRules, delRules, addRules, updateRules } from "@/api/park/rules";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        freeMin: [
          { required: true, message: "免费分钟不能为空", trigger: "blur" }
        ],
        firstMin: [
          { required: true, message: "首段收费时间不能为空", trigger: "blur" }
        ],
        firstMoney: [
          { required: true, message: "首段收费金额不能为空", trigger: "blur" }
        ],
        afterStepMin: [
          { required: true, message: "超过首段间隔时间不能为空", trigger: "blur" }
        ],
        afterStepMoney: [
          { required: true, message: "超过首段收费金额不能为空", trigger: "blur" }
        ],
        lastMin: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除停车场计费规则编号为"' + ids + '"的数据项？').then(function() {
        return delRules(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('park/rules/export', {
        ...this.queryParams
