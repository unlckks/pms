...
public class PayService {
    ...
    // 支付超时，定义为120分钟
    static String timeoutExpress = "5m";
    ...
    public static Map<String,Object> pay(String outTradeNo,
        ...
        AlipayTradePrecreateRequestBuilder builder =new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl(notifyUrl+outTradeNo);
//              .setNotifyUrl("http://www.test-notify-url.com");//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                code=200;
                try {
                    System.out.println("objectMapper.writeValueAsString(res) = " + objectMapper.writeValueAsString(res));
    ...
    /**
     * 退费
     * @param outTradeNo  外部应该交易ID[自己系统的唯一定单号]
...
//                .setNotifyUrl(notifyUrl)//没有用 因为这个回调地址就是收费时的回调地址
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                map.put("outTradNo",response.getOutTradeNo());
                map.put("tradNo",response.getTradeNo());
                try {
